...
import com.apurebase.kgraphql.schema.stitched.DefaultRemoteRequestExecutor
import com.apurebase.kgraphql.schema.stitched.LinkArgument
import com.apurebase.kgraphql.stitched.StitchedSchemaTest.Face
import com.apurebase.kgraphql.stitched.StitchedSchemaTest.Inter
import com.apurebase.kgraphql.stitched.StitchedSchemaTest.InterInter
...
class StitchedSchemaExecutionTest {
    ...
    fun `stitched schema with simple remote query execution should work as expected`() = testApplication {
        ...
        install(GraphQL.FeatureInstance("KGraphql - Local")) {
            endpoint = "local"
            schema {
                configure {
                    remoteExecutor = DefaultRemoteRequestExecutor(client, objectMapper)
                }
                query("local") {
                    resolver { -> "local" }
                }
        ...
        testApplication {
            ...
            install(GraphQL.FeatureInstance("KGraphql - Local")) {
                ...
                schema {
                    ...
                    link(
                        typeName = "RemoteChildChild",
                        fieldName = "stitchedField",
                        remoteQueryName = "remote2"
                    )
                }
            }
    ...
    fun `stitched schema with stitched properties inside fragments should work as expected`() = testApplication {
        ...
        install(GraphQL.FeatureInstance("KGraphql - Local")) {
            ...
            schema {
                ...
                link(
                    typeName = "Remote1",
                    fieldName = "stitchedRemote",
                    remoteQueryName = "remote2",
                    nullable = false
                )
                link(
                    typeName = "Remote1",
                    fieldName = "stitchedLocal",
                    remoteQueryName = "local",
                    localUrl = "/local",
                    nullable = false
                )
            }
    ...
    fun `stitched schema with complex remote mutations should work as expected`() = testApplication {
        ...
        client.post("local") {
            ...
            {"data":{"booleanMutation":{"foo1":"true"},"enumMutation":{"foo1":"REMOTE1"},"stringMutation":{"foo1":"bar1"}}}
        """.trimIndent()
        ...
        install(GraphQL.FeatureInstance("KGraphql - Local")) {
            ...
            schema {
                ...
                link(
                    ...
                    fieldName = "stitchedChild",
                    remoteQueryName = "getRemoteChild",
                    nullable = false,
                    linkArguments = listOf(
                        LinkArgument(
                            name = "bar"
                        )
                    )
    ...
    fun `schema with stitched extension properties should work as expected`() = testApplication {
        ...
        install(GraphQL.FeatureInstance("KGraphql - Local")) {
            ...
            schema {
                ...
                link(
                    typeName = "Remote1",
                    fieldName = "stitchedRemote2",
                    remoteQueryName = "remote2"
                )
                link(
                    typeName = "Remote1",
                    fieldName = "stitchedRemote2Required",
                    remoteQueryName = "remote2",
                    nullable = false
                )
            }
    ...
    fun `schema with stitched enum extension properties should work as expected`() = testApplication {
        ...
        install(GraphQL.FeatureInstance("KGraphql - Local")) {
            ...
            schema {
                ...
                link(
                    typeName = "Remote2",
                    fieldName = "stitchedEnum",
                    remoteQueryName = "remote1"
                )
            }
        }
        ...
        client.post("local") {
            header(HttpHeaders.ContentType, ContentType.Application.Json)
            setBody(
                // TODO: should work *without* bar2 selection; current error: "missing selection on Remote2" because
                //      an empty request is sent to remote2 (as stitchedEnum comes from remote1)
                graphqlRequest(
                    """
                    {
    ...
    fun `schema with stitched extension properties and arguments should work as expected`() = testApplication {
        ...
        install(GraphQL.FeatureInstance("KGraphql - Local")) {
            ...
            schema {
                ...
                link(
                    typeName = "Remote1",
                    fieldName = "stitchedRequiredRemote2",
                    remoteQueryName = "requiredRemote2",
                    nullable = false,
                    linkArguments = listOf(
                        LinkArgument(
                            name = "barValue"
    ...
    fun `schema with stitched list properties should work as expected`() = testApplication {
        ...
        install(GraphQL.FeatureInstance("KGraphql - Local")) {
            ...
            schema {
                ...
                link(
                    typeName = "RemoteWithList",
                    fieldName = "barObjects",
                    remoteQueryName = "remote1",
                    linkArguments = listOf(
                        LinkArgument(
                            name = "foos",
                            parentFieldName = "bars"
                        )
                    )
                )
            }
    ...
    fun `errors from remote execution should be propagated correctly`() = testApplication {
        ...
        client.post("local") {
            ...
            {"errors":[{"message":"don't call me!","locations":[{"line":1,"column":3}],"path":[],"extensions":{"type":"INTERNAL_SERVER_ERROR","detail":{"remoteSchema":"remote","remoteOperation":"fail"}}}]}
        """.trimIndent()
    }
