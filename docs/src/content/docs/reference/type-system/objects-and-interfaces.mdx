...
## Types
The `type` function is the entry point to Type DSL.
...
The extension function `KProperty<T, R>::ignore()` makes KGraphQL ignore its receiver property.
...
The `::transformation(KProperty1<T, R>)` function allows to attach data transformation on any existing Kotlin property.

#### Value transformation

Transformations can alter the value of a property while preserving the type.
...
The `property` function accepts [resolver](../resolver.md) and can be subject of [deprecation](../deprecation.md).
...
### Data Loaded Properties <Badge text={'experimental'} variant={'tip'} />
